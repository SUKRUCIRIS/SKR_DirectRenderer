#include "SKR_DirectRenderer.hpp"
#include <Windows.h>
#include <d2d1.h>
#include <dwrite.h>
#include <wincodec.h>
...
void SKR_WinInit(float VirtualWidth, float VirtualHeight, unsigned long MaxFps, const char* CursorFile, const char* IconFile) {
	WNDCLASSA wc;
	...
	gamewindow = CreateWindowExA(WS_EX_APPWINDOW, wc.lpszClassName, (LPCSTR)u"ÞÜKRÜ ÇÝRÝÞ ENGINE", WS_VISIBLE | WS_POPUP, 0, 0, realwidth, realheight, NULL, NULL, hInstance2, NULL);
...
bool SKR_WinKeyDown(unsigned char VirtualKeyCode) {
...
SKR_WinRect* SKR_CreateWinRect(SKR_Rect* position, SKR_Color* color, float width, float radiusx, float radiusy, bool* condition) {
...
SKR_WinBar* SKR_CreateWinBar(SKR_Rect* position, SKR_Color* bordercolor, SKR_Color* backcolor, SKR_Color* frontcolor, float* maxvalue, float* currentvalue, bool* condition, bool direction) {
...
SKR_WinGameText* SKR_CreateWinGameText(SKR_Rect* position, SKR_Color* color, const wchar_t* string, const wchar_t* font, float size, bool* condition) {
...
SKR_WinGameImage* SKR_CreateWinGameImage(SKR_Rect* dest, SKR_Rect* src, const wchar_t* file, unsigned char flip, bool* condition, float opacity, int animation, unsigned int milliseconds) {
